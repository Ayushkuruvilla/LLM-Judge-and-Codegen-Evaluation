...
class LLMJudgePipeline:
    ...
    def judge_solution(self, prompt, canonical_solution, sample_solution):
        """Evaluate the sample solution using the chosen model."""
        ...
        # Add canonical solution if included (for bias analysis)
        if self.include_canonical_solution:
            evaluation_prompt += (
                f"Canonical objective solution for reference: {canonical_solution}\n"
            )

        # Add final instruction
        evaluation_prompt += "Please keep your response concise and strictly under 500 tokens."

        if self.model_type == "huggingface":
            # Generate the evaluation response using Hugging Face
...
- 4 points in correctness: There are a few minor corrections that must be completed before the code can fulfill the required task.
...
- 4 points in structure: the structure of the code is good, can be understood with a little effort by someone unfamiliar with the code.
...
""")
    ...
    # Evaluate the sample.jsonl file
    input_file = "Data/gemini-1.5-pro_google_temp_0.0.jsonl"
    output_file = "Evaluation/gpt4o_evaluation_gemini_1.jsonl"
